...
void Scene::DrawAgros()
{
    ...
    for (const auto& obj : statics)
    {
        if (obj->Agro())
        {
            Engine::renderer->Draw(obj->Agro(), 0xffff00ff);
            if (obj->Agro()->Type() == POLYGON_T)
                Engine::renderer->Draw(((Poly*)obj->Agro())->BBox(), 0x7fff00ff);
        }
    }
    ...
    for (const auto& obj : moving)
    {
        if (obj->Agro())
        {
            Engine::renderer->Draw(obj->Agro(), 0xff0000ff);
            if (obj->Agro()->Type() == POLYGON_T)
                Engine::renderer->Draw(((Poly*)obj->Agro())->BBox(), 0xff0000ff);
        }
    }
...
bool Scene::AgroCollision(Object* oa, Object* ob)
{
    ...
    if (oa->Agro()) {
        ...
        case CIRCLE_T:
            if (ob->BBox()) {
                ...
                case POINT_T:
                    // Circle && Point
                    return Collision((Circle*)oa->Agro(), (Point*)ob->BBox());

                case CIRCLE_T:
                    // Circle && Circle
                    return Collision((Circle*)oa->Agro(), (Circle*)ob->BBox());

                case RECTANGLE_T:
                    // Circle && Rectangle
                    return Collision((Circle*)oa->Agro(), (Rect*)ob->BBox());

                case POLYGON_T:
                    // Circle && Polygon
                    return Collision((Circle*)oa->Agro(), (Poly*)ob->BBox());

                case MIXED_T:
                    // Circle && Mixed
                    return Collision(oa->Agro(), (Mixed*)ob->BBox());
        ...
        case CIRCLE_T:
            if (oa->BBox()) {
                ...
                case POINT_T:
                    // Circle && Point
                    return Collision((Point*)oa->BBox(), (Circle*)ob->Agro());

                case CIRCLE_T:
                    // Circle && Circle
                    return Collision((Circle*)oa->BBox(), (Circle*)ob->Agro());

                case RECTANGLE_T:
                    // Circle && Rectangle
                    return Collision((Rect*)oa->BBox(), (Circle*)ob->Agro());

                case POLYGON_T:
                    // Circle && Polygon
                    return Collision((Poly*)oa->BBox(), (Circle*)ob->Agro());

                case MIXED_T:
                    // Circle && Mixed
                    return Collision((Mixed*)oa->BBox(), (Circle*)ob->Agro());
...
void Scene::AgroCollisionDetection()
{
    // iteradores para as listas
    list<Object*>::iterator start, end;
