...
Particles::Particles(const Generator & gen) : generator(gen)
{
    sprite = new Sprite(generator.imgFile);
    timer.Start();
}
...
void Particles::Build(float posX, float posY)
{
    ...
    float angle = generator.angle + RandSpread(mt);
    ...
    particle->speed.ScaleTo(RandSpeed(mt));
...
void Particles::Generate(float x, float y, int count)
{
    if (count == 1)
    {
        ...
        if (timer.Elapsed(generator.frequency)) 
        {
            Build(x, y);
            timer.Start();
        }
...
void Particles::Update(float delta)
{
    ...
    while (it != particles.end())
    {
        ...
        lifePercent = timeElapsed/generator.lifetime;

        // torna a partícula transparente com o passar do tempo
        if (lifePercent > generator.percentToDim)
        {
            // transparência cresce cubicamente com o tempo de vida
            generator.color.a = 1.0f - pow(lifePercent, 3.0f);
        }

        if (timeElapsed > generator.lifetime)
        {
...
void Particles::Draw(float z, float factor)
{
    ...
    for (Particle * p : particles)
    {
        ...
        // desenha partícula
        sprite->Draw(p->x, p->y, z, scale, 0, generator.color);
    }    
}
