...
Player* & Missile::player = SoftChase::player;        // referência para o player
...
void Missile::Update()
{
    ...
    if (x > game->Width() || x < 0 || y > game->Height() || y < 0)
    {
        ...
        SoftChase::audio->Volume(HITWALL, level);
        SoftChase::audio->Play(HITWALL);

        // adiciona explosão na cena
        SoftChase::scene->Add(new WallHit(x,y), STATIC);
